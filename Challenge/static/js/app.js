...
// 2. Event listener to detect a change on each input element and call the updateFilters() function.
...
function updateFilters() {
    ...
    // alert("changedElement is" `${changedElement}`);
    ...
    // 5. If a filter value was entered then add that filterId and value to the filters list. Otherwise, clear that filter from the filters object.
      ...
      if (elementValue){
        ...
        // alert("inside if ="+filters[filterId]);
      }
      else {
  ...
  function filterTable() {
    // alert("inside filter table")

    // 8. Set the filtered data to the tableData.
    let filteredData = tableData;

    // console.log("inside filterTable Function = "+filteredData);
